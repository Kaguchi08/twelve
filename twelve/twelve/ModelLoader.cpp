...
ModelLoader::ModelLoader(Renderer& renderer) : dx12_(renderer.GetDx()),
                                               renderer_(renderer) {
}
...
bool ModelLoader::LoadVMDFile(const char* file_name, VMDAnimation* anim) {
    ...
    for (auto& motion : motions) {
        ...
        fread(&motion.frame_no,
              sizeof(motion.frame_no) + sizeof(motion.location) + sizeof(motion.quaternion) + sizeof(motion.bezier),
              1,
              fp);
    }
