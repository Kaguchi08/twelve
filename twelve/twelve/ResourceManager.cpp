...
ResourceManager::ResourceManager(ComPtr<ID3D12Device> dev, std::unique_ptr<class ModelLoader> model_loader) : dev_(dev),
...
void ResourceManager::CreateTextureLoaderTable() {
    mLoadLambdaTable["sph"] = mLoadLambdaTable["spa"] = mLoadLambdaTable["bmp"] = mLoadLambdaTable["png"] = mLoadLambdaTable["jpg"] = [](const std::wstring& path, DirectX::TexMetadata* meta, DirectX::ScratchImage& img)
        -> HRESULT {
        return DirectX::LoadFromWICFile(path.c_str(), DirectX::WIC_FLAGS_NONE, meta, img);
    };

    mLoadLambdaTable["tga"] = [](const std::wstring& path, DirectX::TexMetadata* meta, DirectX::ScratchImage& img)
        -> HRESULT {
        return DirectX::LoadFromTGAFile(path.c_str(), meta, img);
    };

    mLoadLambdaTable["dds"] = [](const std::wstring& path, DirectX::TexMetadata* meta, DirectX::ScratchImage& img)
        -> HRESULT {
