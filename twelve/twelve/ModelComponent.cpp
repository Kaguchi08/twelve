...
void ModelComponent::DrawPMD(bool is_shadow) {
    ...
    cmd_list->SetGraphicsRootDescriptorTable(1, transform_cbv_heap_->GetGPUDescriptorHandleForHeapStart());

    auto material_cbv_heap = pmd_model_->material_cbv_heap.Get();
    ID3D12DescriptorHeap* material_heaps[] = {material_cbv_heap};
...
HRESULT ModelComponent::CreateTransformResourceAndView() {
    ...
    // マップ
    result = transform_const_buffer_->Map(0, nullptr, (void**)&mapped_matrices_);
...
void ModelComponent::MotionUpdate(float delta_time) {
    ...
    for (auto& motion : animation->motion_table) {
        ...
        auto mat = DirectX::XMMatrixTranslation(-pos.x, -pos.y, -pos.z) * rotation * DirectX::XMMatrixTranslation(pos.x, pos.y, pos.z);
...
void ModelComponent::SolveCCDIK(const PMDIK& ik) {
    ...
    for (int c = 0; c < ik.iterations; ++c) {
        ...
        for (int bidx = 0; bidx < bone_positions.size(); ++bidx) {
            ...
            // posを中心に回転
            auto mat = DirectX::XMMatrixTranslationFromVector(-pos) * rot * DirectX::XMMatrixTranslationFromVector(pos);
...
void ModelComponent::SolveLookAt(const PMDIK& ik) {
    ...
    DirectX::XMMATRIX mat = DirectX::XMMatrixTranslationFromVector(-root_pos2) * LookAtMatrix(origin_vec, target_vec, DirectX::XMFLOAT3(0, 1, 0), DirectX::XMFLOAT3(1, 0, 0)) * DirectX::XMMatrixTranslationFromVector(root_pos2);
