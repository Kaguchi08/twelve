...
HRESULT Renderer::CreatePostEffectGraphicsPipeline()
{
	ComPtr<ID3DBlob> vs_blob = nullptr;
	ComPtr<ID3DBlob> ps_blob = nullptr;
	ComPtr<ID3DBlob> error_blob = nullptr;
	...
	gps_desc.SampleDesc.Quality = 0;
	gps_desc.SampleMask = D3D12_DEFAULT_SAMPLE_MASK;
	gps_desc.pRootSignature = post_effect_root_signature_.Get();

	result = dx12_->GetDevice()->CreateGraphicsPipelineState(&gps_desc, IID_PPV_ARGS(post_effect_pipeline_state_.ReleaseAndGetAddressOf()));
...
HRESULT Renderer::CreatePostEffectRootSignature()
{
	...
	range[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 0); // ガウスウェイト
	range[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 0); // フォワードレンダリングまでの結果
	...
	result = dx12_->GetDevice()->CreateRootSignature(0, root_sig_blob->GetBufferPointer(), root_sig_blob->GetBufferSize(), IID_PPV_ARGS(post_effect_root_signature_.ReleaseAndGetAddressOf()));
	if (FAILED(result))
	{
		assert(0);
		return result;
