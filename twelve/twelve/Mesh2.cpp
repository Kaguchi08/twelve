...
namespace
{
	...
	private:
		void ParseMesh(Mesh2& dstMesh, const aiMesh* pSrcMesh);
		void ParseMaterial(Material2& dstMaterial, const aiMaterial* pSrcMaterial);
	};
	...
	bool MeshLoader2::Load(const wchar_t* fileName, std::vector<Mesh2>& meshes, std::vector<Material2>& materials)
	{
		if (fileName == nullptr)
		{
			return false;
	...
	void MeshLoader2::ParseMesh(Mesh2& dstMesh, const aiMesh* pSrcMesh)
	{
		// マテリアル番号を設定
		...
		for (auto i = 0u; i < pSrcMesh->mNumVertices; ++i)
		{
			...
			dstMesh.Vertices[i] = MeshVertex2(
	...
	void MeshLoader2::ParseMaterial(Material2& dstMaterial, const aiMaterial* pSrcMaterial)
	{
		// 拡散反射成分
		{
...
const D3D12_INPUT_ELEMENT_DESC MeshVertex2::InputElements[] = {
...
const D3D12_INPUT_LAYOUT_DESC MeshVertex2::InputLayout = {
	MeshVertex2::InputElements,
	MeshVertex2::InputElementCount
};
static_assert(sizeof(MeshVertex2) == 44, "Vertex struct/layout mismatch");
...
bool LoadMesh(const wchar_t* fileName, std::vector<Mesh2>& meshes, std::vector<Material2>& materials)
{
	MeshLoader2 loader;
